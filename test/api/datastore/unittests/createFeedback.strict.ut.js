...
describe('createFeedback.strict.ut', () => {
    before(function (done) {
        config.set('app:datastore', 'memory');
        config.set('app:validationMode', 'strict');
    ...
    it('should fail to create a new Feedback when userId doesnt exist', (done) => {
        //--GIVEN-------------------------------------------------------------------------------------------------------
        ...
        .end((err, res) => {
            ...
            expect(res.body.error.message).to.equal("User with id="+userId+" does not exist.");
            done();
        });
    });

    it('should fail to create a new Feedback when sessionId doesnt exist', (done) => {
        //--GIVEN-------------------------------------------------------------------------------------------------------
        let userId = "ut-user-1001";
        let sessionId = "ut-sesssion-2001";
        let newFeedback = {
        ...
        .end((err, res) => {
            ...
            expect(res.body.error).to.not.equal(null);
            expect(res.body.error).to.be.a('object');
            expect(res.body.error.message).to.equal("Session with id="+sessionId+" does not exist.");
            done();
        });
    });
    ...
    it('should create a new Feedback when sessionId and userId exist', (done) => {
        //--GIVEN-------------------------------------------------------------------------------------------------------
        let userId = "ut-user-1001";
        let sessionId = "ut-sesssion-2001";
        let feedbackId = 3001;
