...
DataStore.loadConfigs = function() {
    validationMode = config.get('app:validationMode');
    let datastoreConfig = config.get('app:datastore');

    if(datastoreConfig !== 'memory' && datastoreConfig !== 'mysql') {
...
DataStore.getSession = function(req, callback) {
    ...
    callback(true, {message: 'Retrieving a Session is NOT implemented.'});
};

DataStore.updateFeedback = function(req, callback) {
    callback = callback || function(){};
    callback(true, {message: 'Updating a Feedback is NOT implemented.'});
};
...
DataStore.createFeedback = async function(req, callback) {
    ...
    if(validationMode === 'strict') {
        ...
        if(!existsSession) {
            ...
            session.endDate.setDate(session.endDate.getDate() + 1);
...
DataStore.getFeedback = async function(req, callback) {
    callback = callback || function(){};
    let feedbackId = req.params && req.params.feedbackId ? req.params.feedbackId.toString().trim() : "";
    if(!feedbackId) {
        callback(true, {message: 'You must provide a value for `feedbackId` in the url.'});
        return;
    }
    feedbackId = validator.validateInteger(feedbackId);
    if(feedbackId === false || feedbackId < 0) {
        callback(true, {message: 'Invalid value for `feedbackId`, the allowed values are: Any integer greater than 0.'});
        return;
    }
    let response = await ds.getFeedback(feedbackId);
    if(response === null) {
        callback(true, {message: 'An error occurred while attempting to retrieve Feedback.'});
        return;
    } else {
        callback(false, {count: response.length, result: response});
        return;
    }
};
